    ...
    return Container(
      child: ListView(
        ...
        children: [
          ...
          _EmailPasswordForm(), //Kendi yazdığımız bi stateful widget
          //? Google ile giriş
          _SignInProvider(
            // aşağıda kendi yazdığımız bir widget
            infoText: "Google ile giriş yap",
            buttonType: Buttons.Google,
            signInMethod: () async =>
                _signInWithGoogle(), // TODO: Google ile giriş
          ),
          //? Anonim giriş
          _SignInProvider(
...
class __EmailPasswordFormState extends State<_EmailPasswordForm> {
  ...
  //Giriş içi mail methodunu oluşturduk.Burada Try cath içine yazıyoruz.
  void _signIn() async {
    try {
      final UserCredential userCredential =
      ...
      Scaffold.of(context).showSnackBar(SnackBar(
          content: Text(
              'Hoşgeldin ${user.email}'))); //Altta bir bar çıkartır ve bilgilendirme yapar.
